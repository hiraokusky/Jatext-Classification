...
def train(params_set, attention_model, train_loader, criterion, optimizer, epochs=5, use_regularization=False, C=0, clip=False):
    """
        Training code
    ...
    for i in range(epochs):
        ...
        for batch_idx, train in enumerate(train_loader):
            ...
            if use_regularization:
                ...
                identity = Variable(identity.unsqueeze(0).expand(
                    train_loader.batch_size, att.size(1), att.size(1)))
                penal = attention_model.l2_matrix_norm(att@attT - identity)

            if not bool(attention_model.type):
                # binary classification
                # Adding a very small value to prevent BCELoss from outputting NaN's
                correct += torch.eq(torch.round(y_pred.type(
                    torch.DoubleTensor).squeeze(1)), y).data.sum()
                if use_regularization:
                    try:
                        loss = criterion(y_pred.type(torch.DoubleTensor).squeeze(
                            1)+1e-8, y) + C * penal/train_loader.batch_size

                    except RuntimeError:
                        raise Exception(
                            "BCELoss gets nan values on regularization. Either remove regularization or add very small values")
                else:
                    loss = criterion(y_pred.type(
                        torch.DoubleTensor).squeeze(1), y)

            else:
                if params_set['verbose']:
                    # 予測値と期待値を表示
                    print('\t', torch.eq(torch.max(y_pred, 1)[1], y.type(torch.LongTensor)).data.sum(
                    ), torch.max(y_pred, 1)[1], y.type(torch.LongTensor))

                correct += torch.eq(torch.max(y_pred, 1)
                                    [1], y.type(torch.LongTensor)).data.sum()
                if use_regularization:
                    loss = criterion(
                        y_pred, y) + (C * penal/train_loader.batch_size).type(torch.FloatTensor)
                else:
            ...
            if clip:
                torch.nn.utils.clip_grad_norm_(
                    attention_model.parameters(), 0.5, norm_type=2)
            optimizer.step()
            n_batches += 1
        ...
        print("{}: step {}, loss {:g}, acc {:g}".format(time_str, i+1, total_loss /
                                                        n_batches, float(correct) / (n_batches*train_loader.batch_size)))
...
def evaluate(attention_model, x_test, y_test):
    ...
    if bool(attention_model.type):
        ...
        y_test_var = Variable(torch.from_numpy(
            y_test).type(torch.DoubleTensor))
